...
params_check = "modelfiles/tta_rdrop_n"
params_batch_size = 4
...
def metrics(logits, label):
    ...
    labelc = tf.tile(labelexpand, [2, 1])

    loss = tf.reduce_mean(binary_crossentropy(labelc, logits))

    logits1, _ = tf.split(logits, 2)
...
def train_step(data, model, optimizer):
    with tf.GradientTape() as tape:
        logits, _ = model(tf.tile(data["sen"], [2, 1]), training=True)
...
def dev_step(data, model):
    logits, _ = model(tf.tile(data["sen"], [2, 1]), training=False)
    loss, accsum, num, tp, tn, fp = metrics(logits, data["label"])
...
class TextClassify():
    ...
    def testc(self, test_file):
        ...
        for data in test_batch:
            _, predict = model.predict(data["sen"])

            tp_ = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(data["label"], 1), tf.equal(predict, 1)), tf.float32))
            tn_ = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(data["label"], 1), tf.equal(predict, 0)), tf.float32))
            fp_ = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(data["label"], 0), tf.equal(predict, 1)), tf.float32))
